...
bad_strings <- prepped |> mutate(has_bad=map_lgl(text, is_bad)) |> filter(has_bad==T)
...
prepped |> nest(text=c(role, playerId, text)) |> nrow() #787

bad_strings |>  nest(text=c(role, playerId, text)) |> nrow() #16
...
```{r}
good_strings <- prepped |> mutate(has_bad=map_lgl(text, is_bad)) |> filter(has_bad==F) |> 
  select(-has_bad) |> 
  nest(text=c(role, playerId, text)) 
