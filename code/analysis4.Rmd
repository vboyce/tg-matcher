...
dat_loc <- "data/pilottgmatcheryoked-trials.csv"
...
ggplot(spr |> filter(rt<2000), aes(x=rt, fill=workerid))+geom_histogram()+geom_vline(aes(xintercept=100))
...
selections |> ggplot(aes(x=orig_repNum, y=button_rt/1000, color=condition))+geom_jitter(alpha=.5)+stat_summary()+geom_hline(yintercept=3)

selections |> ggplot(aes(x=trial_index, y=button_rt/1000, color=condition))+geom_jitter(alpha=.5)+geom_smooth()+geom_hline(yintercept=3)


selections |> group_by(workerid, orig_repNum, condition) |> summarize(pct_correct=mean(correct)) |> ggplot(aes(x=orig_repNum, y=pct_correct, color=condition))+geom_jitter()

selections |> group_by(workerid, trial_index, condition) |> summarize(pct_correct=mean(correct)) |> ggplot(aes(x=trial_index, y=pct_correct, color=condition))+geom_point()+geom_smooth()
