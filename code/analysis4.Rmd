...
blah <- fixef(foo) |>
  ...
  mutate(
    ...
    lag = case_when(
      ...
      type %in% c("LLM", "VLM", "KL", "freq x len", "freq", "length") ~ "word n",
      T ~ NA
    )
  ) |>
  mutate(
    type = factor(type, levels = c("length", "freq", "freq x len", "LLM", "VLM", "KL", "conditionyoked", "orig_repNum", "orig_repNum:conditionyoked", "matcher_trialNum")),
    type = fct_rev(type)
  )
...
blah |>
  ...
  labs(x = "Estimated coefficient", color = "Predictor lag") +
  theme(
...
ggsave(here("writing/word_preds.pdf"), width = 5, height = 3)
