...
```{r}
kl_pred <- read_csv(here(pred_loc, "kl_preds.csv")) |>
  # select(-text, -partial, -condition, -trialNum) |>
  select(-condition, -trialNum, -text) |>
...
llm_concat_probs <- read_csv(here(pred_loc, "llm_probs_concat.csv")) |>
  ...
  select(correct_tangram, gameId, orig_repNum = repNum, word_speaker_index, surprisal_llm_concat, word)

vlm_concat_probs <- read_csv(here(pred_loc, "vlm_probs_concat.csv")) |>
  mutate(word_speaker_index = word_idx + 1, words = full_text) |>
  left_join(read_csv(here(pred_loc, "expt_4_games_concat.csv"))) |>
  mutate(surprisal_vlm_concat = logprob * log2(exp(1)) * -1) |>
  select(correct_tangram, gameId, orig_repNum = repNum, word_speaker_index, surprisal_vlm_concat, word)
...
spr_labeled <- spr |>
  ...
  mutate(across(c("freq", "word_len", "surprisal_llm_concat", "surprisal_vlm_concat", "kl"), list(lag = ~ lag(.x), lag2 = ~ lag(.x, 2), lag3 = ~ lag(.x, 3)))) |>
  ungroup() |>
...
spr_labeled |>
  filter(is.na(surprisal_vlm_concat)) |>
  filter(!is.na(word_speaker_index))
spr_labeled |>
  filter(is.na(surprisal_llm_concat)) |>
  filter(!is.na(word_speaker_index))
spr_labeled |>
  filter(is.na(freq)) |>
  filter(!is.na(word_speaker_index))
spr_labeled |>
  filter(is.na(word_len)) |>
  filter(!is.na(word_speaker_index))
spr_labeled |>
  filter(is.na(kl)) |>
  filter(!is.na(word_speaker_index)) # This one says we need to fix things!!!!
```
