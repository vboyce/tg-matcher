...
## but first what's the speed -- accuracy look like 

```{r}
speed_acc <- good_stuff |>
  filter(rt_sec < 60) |>
  group_by(workerid, correct) |>
  summarize(mean_rt = mean(rt_sec), n = n())

ggplot(speed_acc, aes(x = workerid, y = mean_rt, alpha = n, color = as.factor(correct))) +
  geom_point()

speed_acc |>
  select(-n) |>
  pivot_wider(names_from = correct, values_from = mean_rt) |>
  filter(`1` < `0`) # for 53/60 correct answers are faster on average
```

## playing around

in which we conclude that MLE sucks

```{r}
test <- good_stuff |>
  filter(rt_sec < 60) |>
  select(correct, q = rt_sec) |>
  mutate(resp = ifelse(correct == 1, "upper", "lower")) |>
  select(resp, q)

library(RWiener)

foo <- wdm(test, beta = .5, fixed = 1)
```

```{r}
# try the 4 quadrants

small_early <- good_stuff |>
  filter(rt_sec < 60) |>
  filter(round == "round_1") |>
  filter(group_size == "2_player") |>
  select(correct, q = rt_sec) |>
  mutate(resp = ifelse(correct == 1, "upper", "lower")) |>
  select(resp, q)

wdm(small_early, beta = .5) # very high nonstability

small_late <- good_stuff |>
  filter(rt_sec < 60) |>
  filter(round == "round_6") |>
  filter(group_size == "2_player") |>
  select(correct, q = rt_sec) |>
  mutate(resp = ifelse(correct == 1, "upper", "lower")) |>
  select(resp, q)

wdm(small_late, beta = .5)
```
## Back to bayesian, but toy

```{r}
library(RWiener)

test <- good_stuff |> filter(rt_sec < 60)


formula <- bf(
  rt_sec | dec(correct) ~ 0 + group_size:round,
  bs ~ 1,
  ndt ~ 1,
  bias = .5
)
