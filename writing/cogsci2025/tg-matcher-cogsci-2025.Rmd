...
We recruited `r expt_1_data |> select(workerid) |> unique() |> nrow()` participants who each saw 60 trials (15 in each of the 4 conditions). Overall, participants saw `r expt_1_data |> select(gameId, round, correct_tangram) |> unique() |> nrow()` transcripts from `r expt_1_data |> select(gameId) |> unique() |> nrow()` games. This experiment was pre-registered at https://osf.io/k45dr. 
...
For experiment 2b, we recruited `r expt_2_data |> select(workerid) |> unique() |> nrow()` participants who each saw 64 trials (8 in each of the 8conditions). Overall, participants saw `r expt_2_data |> select(gameId, round, correct_tangram) |> unique() |> nrow()` transcripts from `r expt_2_data |> select(gameId) |> unique() |> nrow()` games.  This experiment was pre-registered at  https://osf.io/rdp5k. 
