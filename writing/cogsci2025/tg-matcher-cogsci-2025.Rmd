...
do_preds_tangram_yoked <- function(model) {
  ...
  preds <- expand_grid(
    ...
    correct_tangram = c("A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L")
     ...
     add_linpred_draws(mod, value = "predicted", re_formula = ~ (1| correct_tangram)) |>
    # group_by(group_size, thickness, round, correct_tangram) |>
  ...
  pred_acc_1 <- do_preds_tangram_yoked("acc_4.rds") |> write_rds(here(mod_loc, "predicted", "acc_4_tangram.rds"))
...
mods_me <- c(
  "acc_1.rds", "acc_2.rds", "acc_4.rds", "acc_mlp_1_2.rds", "acc_yoked_mlp.rds", "yoked_shuffled_original.rds"
) |> walk(~ do_me(.))
...
```{r fig-calibration, fig.env="figure", fig.pos = "t", fig.align = "center", out.width="100%", fig.width=5, fig.height=3, fig.cap = "TODO Make all pretty \\label{FOOBAR}" }
...
```{r fig-1, fig.env="figure", fig.pos = "t", fig.align = "center", out.width="100%", fig.width=5, fig.height=5, fig.cap = "TODO make prettier \\label{TODO}" }
expt_2_fig_data |> filter(source=="naive") |> ggplot(aes(x = round, color = source, y = correct, group = interaction(source, group_size), shape=group_size, lty=group_size)) +
    # geom_jitter(data = expt_2_fig_data |> filter(source=="naive") |> group_by(source, group_size, round, correct_tangram, thickness, expt) |>    summarize(correct = mean(correct)), color="black", alpha=.2, width=.2) +
  ...
  stat_summary(data= expt_2_fig_data |> filter(source!="naive"),  geom="point", position=position_dodge(.4))+
    stat_summary(data= expt_2_fig_data |> filter(source!="naive"), geom="line", position=position_dodge(.4))+
...
```{r fig-2, fig.env="figure", fig.pos = "t", fig.align = "center", out.width="100%", fig.width=5, fig.height=5, fig.cap = "TODO We may want to add model predictions onto this as well may also want to use model predictions b/c error bars don't capture mixed effects \\label{TODO2}" }
library(ggtext)
correct_tangram <- c("A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L")
...
ggplot(acc_by_type_target, aes(x = correct_tangram, y = acc, color=source, shape=group_size, lty=group_size)) +
   stat_summary(data = tangram_predicted_expt_2_fig, aes(y = mean, ymax = high, ymin = low), fun.data="mean_cl_boot", position=position_dodge(width=.3)) +
   #geom_pointrange(data = tangram_predicted_expt_2_fig, aes(y = mean, ymax = high, ymin = low)) +
  stat_summary(data=acc_by_type_target |> filter(source!="naive"), aes(color = source), geom="point", position=position_dodge(width=.3)) +
...
acc_mod_mlp_orig_acc <- brm(
  correct ~ original_correct + group_size * thickness * round +
    (group_size * thickness * round | correct_tangram),
  data = expt_2_mlp,
  file = here(mod_loc, "acc_mlp_1_2_orig_acc"),
  prior = acc_priors_mlp,
  control = list(adapt_delta = .95)
)
...
acc_mod_mlp_orig_length <- brm(
  correct ~ log_words + group_size * thickness * round +
    (group_size * thickness * round | correct_tangram),
  data = expt_2_mlp,
  file = here(mod_loc, "acc_mlp_1_2_orig_length"),
  prior = acc_priors_mlp,
  control = list(adapt_delta = .95)
)
```
...
predicted_expt_3_fig <- read_rds(here(mod_loc, "predicted/acc_4_tangram.rds")) |> mutate(across(mean:high, inv_logit_scaled))|> mutate(round=str_c("round_", orig_repNum+1))

expt_3_fig_data |> ggplot(aes(x = round, color = condition, y = correct, group = condition)) +
  #geom_point(data = expt_3_fig_data |> group_by(condition, round, correct_tangram) |>
...
acc_mod_yoked_mlp <- brm(correct ~ orig_repNum + (1 | gameId) + (1 | correct_tangram),
  ...
  prior = acc_priors_mlp,
  file = here(mod_loc, "acc_yoked_mlp"),
  control = list(adapt_delta = .95)
)
```
